...
Работает через `python serve.py`.
...
2. Создать окружение и установить зависимости:
   ...
   Все команды ниже выполняются в активированном окружении: в нём доступна команда `python`. Точные версии проверенного окружения ставятся командой `pip install -r requirements.lock` (нужен Python 3.12+: закреплённый в нём networkx 3.7 не ставится на 3.11).

3. Запустить полный пересчёт — **одна команда**:

   ```bash
   python run.py                      # data/*.parquet → out/
   python run.py --open               # то же и сразу открыть экран в браузере
   python run.py --data /путь/к/parquet --out out-new   # другая выгрузка той же схемы
   ```

   Первый запуск после установки может идти дольше из-за первичной загрузки библиотек.

4. Открыть экран просмотра `out/index.html` в браузере. Он работает без сервера и без интернета.

5. Запустить экран с AI-ассистентом:

   ```bash
   python serve.py                    # http://localhost:8000, офлайн-режим ассистента
   OPENAI_API_KEY=sk-... python serve.py                 # LLM-агент, модель по умолчанию gpt-5.4
   OPENAI_API_KEY=sk-... OPENAI_MODEL=<модель> python serve.py
   OPENAI_BASE_URL=http://localhost:11434/v1 OPENAI_API_KEY=x OPENAI_MODEL=<модель> python serve.py   # OpenAI-совместимый сервер
   ```

   Параметры: `--port` (по умолчанию 8000; если порт занят, берётся следующий свободный), `--data`, `--out`, `--no-open`.
...
6. Запустить тесты:
   ...
   node --test tests/test_review.cjs      # 4 теста, Node.js 22
   ```
...
2. **Выгрузки.** Проверка числа строк:
